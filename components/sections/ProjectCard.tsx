...
export function ProjectCard({ project }: { project: Project }) {
  ...
  return (
    <TiltCard max={6} wrapperClassName="h-full" className="h-full rounded-xl">
      <motion.article
        whileHover={{ y: -6 }}
        transition={{ type: "spring", stiffness: 300, damping: 24 }}
        onPointerMove={trackSpotlight}
        className="spotlight hover-glow group flex h-full flex-col overflow-hidden rounded-xl border border-border bg-card transition-colors hover:border-primary/40"
      >
        {/* Imagen */}
        ...
        <div className="flex flex-1 flex-col p-5">
          <div className="flex items-start justify-between gap-3">
            <h3 className="text-base font-semibold tracking-tight transition-colors duration-300 group-hover:text-primary">
              {project.title}
            </h3>
            <span
              className={cn(
